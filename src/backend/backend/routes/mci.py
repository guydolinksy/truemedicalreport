...
from common.mci import MCI_NAMES, MCIIntakeWing
...
async def create_anonymous_patient(arrival: str = Body(..., embed=True), department: str = Body(..., embed=True),
                                   wing: Optional[str] = Body(default=None, embed=True)):
    from faker import Faker
    faker = Faker('he-IL')
    external_id = f"mci#{arrival}"
    patient = Patient(version=0, source_identity='manually', external_id=external_id,
                      info=Person(id_='000000000', name=f"{faker.word()} {faker.color_name()}"),
                      admission=Admission(arrival=arrival, department_id=department, wing_id=wing))
