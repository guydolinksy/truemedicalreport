...
const Image = ({imageName, imageLink, at}) => {
    ...
    return <div>
        <a target="_blank" rel="noopener noreferrer" href={imageLink}> {imageName} - <Moment date={at}
                                                                                             format={'DD-MM-YYYY'}/></a>
    </div>
}
const Images = ({patient}) => {
    ...
    useEffect(() => {
        axios.get(`/api/images/${patient}`).then(response => {
...
const Surgeries = ({patient}) => {
    ...
    useEffect(() => {
        axios.get(`/api/surgeries/${patient}`).then(response => {
...
const AllSurgeries = ({patientId}) => {
    ...
    useEffect(() => {
        axios.get(`/api/surgeries/${patientId}`).then(response => {
            setSurgeries(response.data)
        })
    }, [patientId])
    return surgeries.length ?
        <>
            ...
            <Modal title="כל הניתוחים" open={isModalOpenSurgeries} onOk={handleOk}
            ...
            <div>
                <a onClick={() => showModal(patientId)}>הצג את כלל הניתוחים של המטופל</a>
            </div>
        </>
        :
...
const AllImages = ({patientId}) => {
    ...
    useEffect(() => {
        axios.get(`/api/images/${patientId}`).then(response => {
            setImages(response.data)
        })
    }, [patientId])
...
export const Results = () => {
    ...
    const updateData = useCallback(() => {
        ...
        axios.get(`/api/trauma/records`, {}).then(response => {
    ...
    const filteredData = dataSource.filter(
        item => [undefined, null, ''].includes(filterID) || item.id.toString().includes(filterID)
    ).filter(
        item => [undefined, null, ''].includes(filterName) || item.name.toString().includes(filterName)
    ).filter(
    ...
    return <>
        <Table size={"small"} pagination={filteredData.length > 100 ? {pageSize: 100} : false}
               style={{flex: 1, height: '100%', zIndex: 2, border: 'black'}} loading={loadingData}
               dataSource={filteredData} scroll={{y: '100%'}} bordered columns={[
            {
                title: () => <span>ת.ז.&nbsp;&nbsp;
                    <Input allowClear onChange={e => setFilterID(e.target.value)}/></span>,
                key: "id",
                width: '12%',
                render: row => <div>
                    <a target="_blank" rel="noopener noreferrer"
                       href={`https://timeline.arc-prod.sheba.gov.il/timeline/${row.id}`}>{row.id}</a>
                </div>


            }, {
                title: () => <span>שם&nbsp;&nbsp;
                    <Input allowClear onChange={e => setFilterName(e.target.value)}/></span>,
                dataIndex: "name",
            }, {
            ...
            {
                title: () => <span>מחלקה נוכחית&nbsp;&nbsp;&nbsp;
                    <Select allowClear showSearch placeholder={'בחר.י מחלקה נוכחית'} loading={loadingData}
                            value={filterDepartmentName} onChange={setFilterDepartmentName} style={{width: '150px'}}
                            options={unique(dataSource.map(({DepartmentName: department}) => department)).map(baseUnit => ({
                                label: baseUnit,
                                value: baseUnit
                            }))}
                            filterOptions={(input, option) => option.label.includes(input)}/></span>,
            ...
            , {
                ...
                key: "DepartmentNames",
                render: row => <div
                    style={{whiteSpace: "pre-line"}}>{row.DepartmentNames.split(',').slice(1).join("\n")}</div>
            },
            {
