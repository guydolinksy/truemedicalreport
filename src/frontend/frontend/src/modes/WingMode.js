...
export const WingModeInner = ({ onError, style }) => {
  ...
  const patients = useMemo(() => {
    return value.getPatients.patients
      .filter(
        ({ oid }) =>
          !selectedAwaiting.filter((filter) => value.getWings.wings[0].filters.mapping[filter] !== undefined).length ||
          selectedAwaiting.find((filter) => (value.getWings.wings[0].filters.mapping[filter] || []).includes(oid)),
      )
      .filter(
        ({ oid }) =>
          !selectedTreatments.filter((filter) => value.getWings.wings[0].filters.mapping[filter] !== undefined)
            .length ||
          selectedTreatments.find((filter) => (value.getWings.wings[0].filters.mapping[filter] || []).includes(oid)),
      )
      .filter(
        ({ oid }) =>
          !selectedDoctors.filter((filter) => value.getWings.wings[0].filters.mapping[filter] !== undefined).length ||
          selectedDoctors.find((filter) => (value.getWings.wings[0].filters.mapping[filter] || []).includes(oid)),
      )
      .filter(
        ({ oid }) =>
          !selectedTime.filter((filter) => value.getWings.wings[0].filters.mapping[filter] !== undefined).length ||
          selectedTime.find((filter) => (value.getWings.wings[0].filters.mapping[filter] || []).includes(oid)),
      )
...
export const WingMode = ({ onError, style }) => {
  ...
  return (
    <wingDataContext.Provider
      url={`/api/departments/${curView.department_id}/wings/${curView.wing_id}`}
      onError={onError}
    >
      {() => (
        <>
