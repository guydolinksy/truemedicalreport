...
const LayoutModeInner = ({ onError }) => {
  ...
  return (
    ...
    >
      {!isFiltered &&
        value.getWings.wings[0].details.layout &&
        value.getWings.wings[0].details.layout.beds.map((bed, i) => {
          if (bed === null) return <div key={i} />;
          return (
            <Bed
              key={i}
              admission={{
...
export const LayoutMode = ({ onError }) => {
  ...
  return (
    <layoutDataContext.Provider
      url={`/api/departments/${curView.department_id}/wings/${curView.wing_id}/layout`}
      onError={onError}
    >
      {() => (
        <>
