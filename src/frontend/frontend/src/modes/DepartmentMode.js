...
import { InfoContext, InfoDrawer } from '../components/panel/Info';

const { Content } = Layout;
export const departmentDataContext = createDataContext();
const hideSetting = (e, wing, userSettings, setting) => {
  ...
  Axios.post('/api/settings/statistics', {
    statistics: Object.assign(
      {},
      ...Object.entries(userSettings.statistics)
        .filter(([k, v]) => k !== wing.key)
        .map(([k, v]) => ({ [k]: v })),
...
export const DepartmentMode = ({ onOk }) => {
  ...
  return (
    <departmentDataContext.Provider url={`/api/departments/${curView.department_id}`}>
      {({ value }) => (
        <Content>
              ...
              >
                <div style={{ overflowY: 'auto' }}>
                  <Space style={{ justifyContent: 'center' }} wrap>
                    {value.getWings.wings
                      .filter((wing) => wing.patients_count > 0)
                      .map((wing) => {
                        const filter = value.getViewFilters.filters.find((filter) => filter.key == wing.key);
                        const actions = []
                          .concat(
                            ...filter.doctors.map(toActions),
                            ...filter.awaiting.map(toActions),
                            ...filter.treatments.map(toActions),
                            ...filter.time_since_arrival.map(toActions),
                          )
                          .filter(({ key }) => {
                            ...
                            if (filters.some(([regex, result]) => result && key.match(regex))) return true;
                            return !filters.some(([regex, result]) => !result && key.match(regex));
                          })
                          .map(({ count, title, icon, duration, valid, key }) => (
                            <Space
                              key={key}
                              style={{
                                alignItems: 'center',
                            ...
                            >
                              <CloseCircleOutlined
                                className="departmentActionItem"
                                onClick={(e) => {
                              ...
                              />
                              <div
                                className={valid || !count ? undefined : 'error-text'}
                              ...
                              <div style={{ fontSize: 12 }}>
                                {icon && (
                                  <span>
                                    &nbsp;
                                    <CustomIcon status={'processing'} icon={icon} />
                                  </span>
                                )}
                              </div>
                        ...
                        const untreated = ((filter.mapping.find((m) => m.key === 'treatment.ללא') || {}).values || [])
                            .length,
                          discharged = ((filter.mapping.find((m) => m.key === 'treatment.שחרור') || {}).values || [])
                            .length,
                          count = wing.patients_count,
                          hospitalized = count - untreated - discharged;
                        return (
                          <Col key={wing.key} span={12}>
                            <Card
                              title={
                                <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
                                  ...
                                  >
                                    <Progress
                                      percent={(100 * (discharged + hospitalized)) / count}
                              ...
                              hoverable
                              onClick={() => {
                                if (curView.wing_id === wing.key && onOk) onOk();
                                else
                                  navigate(
                                    generatePath(MODE_URL, {
                                      viewType: 'wing',
                                      view: viewsValue.getViews.views.find(
                                        (v) => v.department_id === curView.department_id && v.wing_id === wing.key,
                                      ).key,
                                      mode: 'patients',
                                    }),
                                  );
