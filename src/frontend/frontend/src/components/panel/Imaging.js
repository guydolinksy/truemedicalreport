    ...
    <Card
      title={
        <div style={{ width: '100%', display: 'flex', flexFlow: 'row nowrap', justifyContent: 'space-between' }}>
          <span>הדמיות</span>
          <div>
            <Badge style={{ backgroundColor: '#1890ff' }} count={Object.keys(value.imaging).length} size={'small'} />
          </div>
        </div>
      }
    >
      {modalContext}
      {Object.keys(value.imaging).length ? (
        Object.values(value.imaging).map((image, i) => (
          <p
            key={i}
            style={{
              animation: matched(['info', patient, 'imaging', `${image.external_id}`])
