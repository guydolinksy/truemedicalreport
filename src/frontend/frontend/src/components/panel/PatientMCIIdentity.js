...
const PatientMCIIdentityInner = () => {
  ...
  const hasData = useMemo(() => {
    return (
      !value.mci.diagnosis.length ||
      !value.mci.pre_hospital_treatment.length ||
      !value.mci.hospital_treatment.length ||
      !value.mci.imaging.length
    );
  }, [value]);
  const deletePatient = () => {
  ...
  if (value.source_identity === 'manually')
    return (
      <div style={{ display: 'flex', flex: 1, minWidth: 'fit-content' }}>
        <Button danger icon={<DeleteOutlined />} type={'primary'} onClick={deletePatient} disabled={hasData} />
        <Select
          value={mergeTo}
          onChange={setMergeTo}
          disabled={!hasData}
          options={unmergedPatients}
          style={{ flex: 1 }}
          dropdownStyle={{ minWidth: 300 }}
          placement={'bottomLeft'}
        />
        <Button icon={<MergeCellsOutlined />} type={'primary'} onClick={mergePatient} disabled={!hasData || !mergeTo} />
      </div>
    );
  return (
    <div>
