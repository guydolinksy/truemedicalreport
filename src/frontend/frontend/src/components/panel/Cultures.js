...
const displayCulture = (culture, matched, patient) => {
    ...
    if (ranges.some(range => ["HH", "LL"].includes(range)))
        ...
        badgeColor = "#FFA500";
    else if (culture.status === 4 && ranges.every(range => range === "N"))
        badgeColor = "#00FF00";

    const rangesToConsiderAsBad = ["HH", "LL", "VH", "VL", "H", "L"].map(
    ...
    return <p key={`${culture.category}-${culture.ordered_at}`} style={{
        animation: matched(['panel', patient, 'cultures', encodeURIComponent(culture.category), culture.ordered_at.replace(/:/g, '-').replace(/\+/g, '-')]) ? 'highlight 2s ease-out' : undefined,
        direction: "rtl"
    }}>
        <p>
            <span><Badge style={{backgroundColor: badgeColor}} count={badgeText}/>&nbsp;
                {culture.category_display_name} {cultureStatuses[culture.status]} {ranges.some(range => range === "X") && ' - פסול'}
        ...
        <p style={{marginRight: "2rem"}}>
            ...
            {(rangeToResults[null] || []).length > 0 && <p>
