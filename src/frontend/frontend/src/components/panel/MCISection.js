  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {item.value && (
        <TimePicker
          inputReadOnly
          value={item.at ? moment(item.at) : null}
          format={'HH:mm'}
          onSelect={(newTime) => {
            update(
              [MCI_DEPARTMENT, sectionKey],
              [
                ...
                {
                  value: item.value,
                  at: (moment().isBefore(newTime) ? newTime.subtract(1, 'days') : newTime)
                    .toISOString()
                    .replace('Z', '+00:00'),
                },
