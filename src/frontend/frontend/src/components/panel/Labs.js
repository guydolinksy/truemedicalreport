...
const displayLab = (lab, displayAllResults, matched, patient) => {
  ...
  else if (lab.status === 4 && ranges.every((range) => range === 'N')) badgeColor = '#00FF00';

  const rangesToConsiderAsBad = ['HH', 'LL', 'VH', 'VL']
    .concat(displayAllResults ? ['H', 'L'] : [])
    .map((status) => rangeToResults[status] || []);
  ...
  return (
    <p
      key={`${lab.category}-${lab.ordered_at}`}
      style={{
        animation: matched([
          'info',
          patient,
          'labs',
          encodeURIComponent(lab.category),
          lab.ordered_at.replace(/:/g, '-').replace(/\+/g, '-'),
        ])
    ...
    >
      ...
      <p style={{ marginRight: '2rem' }}>
        ...
        {displayAllResults && (rangeToResults[null] || []).length > 0 && (
    ...
    <Card
      header={
        <div style={{ width: '100%', display: 'flex', flexFlow: 'row nowrap', justifyContent: 'space-between' }}>
          <span>מעבדה</span>
          <Space align="center">
            <Badge style={{ backgroundColor: '#1890ff' }} count={Object.keys(value.labs).length} size={'small'} />
