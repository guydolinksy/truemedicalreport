import {Badge, Button, Collapse, Input, Empty, List, Modal, Radio, Space, Spin} from "antd";
...
export const PatientInfo = ({setHeader, onError}) => {
...
const InternalPatientCard = ({patient, setHeader}) => {
    ...
    return <>
        <Input value={value.comment} onChange={e => update(['comment'], e.target.value, false)} placeholder={'הערות:'}
               style={{width: '100%'}}/>
        <Collapse defaultActiveKey={['basic'].concat(...matching(['info', patient]).slice(0, 1))}>
            ...
            <Panel key={'notifications'} header={
                <div style={{width: '100%', display: "flex", flexFlow: "row nowrap", justifyContent: "space-between"}}>
                    <span>עדכונים</span>
                    <div><Badge style={{backgroundColor: '#1890ff'}} count={Object.keys(value.notifications).length}
                                size={"small"}/></div>
                </div>} style={{
                animation: matched(['info', patient, 'notifications']) ? 'highlight 2s ease-out' : undefined
            }}>
                {Object.keys(value.notifications).length > 0 ? Object.values(value.notifications).map((notification, i) =>
            ...
            <Panel key={'referrals'} header={
                <div style={{width: '100%', display: "flex", flexFlow: "row nowrap", justifyContent: "space-between"}}>
                    <span>ייעוץ</span>
                    <div><Badge style={{backgroundColor: '#1890ff'}} count={Object.keys(value.referrals).length}
                                size={"small"}/></div>
                </div>}>
                {Object.keys(value.referrals).length ? Object.values(value.referrals).map((referral, i) => <p key={i}>
                    {referral.to} - {referral.completed_at ? 'הושלם' : 'בהמתנה'} - <RelativeTime style={{fontSize: 12}}
                                                                                                 date={referral.at}/>
