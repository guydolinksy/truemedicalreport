...
const PatientsFilterInner = ({children}) => {
    ...
    const {filteredPatients, isFiltered} = useMemo(() => {
        ...
        const relevantTime = selectedTime.filter(
            filter => value.getViewFilters.filters[0].mapping[filter] !== undefined
        )
        return {
            filteredPatients:
                value.getPatients.patients.filter(({oid}) => !relevantAwaiting.length || relevantAwaiting.find(
                    filter => (value.getViewFilters.filters[0].mapping[filter] || []).includes(oid)
                )).filter(({oid}) => !relevantTreatments.length || relevantTreatments.find(
                    filter => (value.getViewFilters.filters[0].mapping[filter] || []).includes(oid)
                )).filter(({oid}) => !relevantDoctors.length || relevantDoctors.find(
                    filter => (value.getViewFilters.filters[0].mapping[filter] || []).includes(oid)
                )).filter(({oid}) => !relevantTime.length || relevantTime.find(
                    filter => (value.getViewFilters.filters[0].mapping[filter] || []).includes(oid)
    ...
    return <filterContext.Provider value={{filteredPatients, isFiltered}}>
        {children}
        <Popover placement={"leftTop"} title={"סינון תצוגת אגף"}
                 content={<div>
                     <div style={filterTagsContainerStyle}>
                         <b style={{whiteSpace: "nowrap"}}>רופא.ה:</b>
                         {value.getViewFilters.filters[0].doctors.map(filter => <CheckableTag
                             key={filter.key}
                     ...
                     <div style={filterTagsContainerStyle}>
                         ...
                         {value.getViewFilters.filters[0].treatments.map(filter => <CheckableTag
                             key={filter.key}
                     ...
                     <div style={filterTagsContainerStyle}>
                         ...
                         {value.getViewFilters.filters[0].time_since_arrival.map(filter => <CheckableTag
                             key={filter.key}
                     ...
                     <Divider style={{marginTop: 10, marginBottom: 10}}/>
                     <Tree treeData={value.getViewFilters.filters[0].awaiting.map(toTree)} style={{width: '100%'}}
                           checkable multiple
...
export const PatientsFilter = ({children, onError}) => {
    ...
    return <filtersDataContext.Provider url={`/api/views/${viewType}/${view}/filters`} onError={onError}>
        {() => <PatientsFilterInner>
            {children}
        </PatientsFilterInner>}
