  ...
  return (
    <Popover
      ...
      content={
        <div style={{ textAlign: 'center' }}>
          <div>
            <span
              className={!data || data.is_valid === null ? undefined : data.is_valid ? 'ok-text' : 'error-text'}
              style={{ userSelect: 'none', fontSize: 14 }}
            >
              <b>{data && data.value ? dataValue : '-'}</b>
            </span>
            &nbsp;
            {data && data.effect && data.effect.kind && (
    ...
    >
      <div
        ...
        style={{ userSelect: 'none', flexWrap: 'nowrap', display: 'flex' }}
        onClick={
          data
            ? (e) => {
      ...
      >
        ...
        {data && data.effect && data.effect.kind && (
          <span>
            ...
            <CustomIcon
              style={{ fontSize: 12 }}
              status={data.is_valid !== false ? 'processing' : 'error'}
  ...
  return (
    <div
      style={{
        ...
        padding: '8px 12px',
        maxHeight: 160,
        width: hover || isTablet ? '100%' : 40,
        flex: 0,
      }}
      onMouseEnter={() => setHover(true)}
      onMouseLeave={() => setHover(false)}
    >
      <Measure patient={patient} hover={hover || isTablet} measure={'temperature'} icon={'temperature'} title={'חום'} />
      <Measure
        patient={patient}
        hover={hover || isTablet}
        measure={'blood_pressure'}
        icon={'bloodPressure'}
        title={'לחץ דם'}
      />
      <Measure patient={patient} hover={hover || isTablet} measure={'pulse'} icon={'pulse'} title={'דופק'} />
      <Measure
        patient={patient}
        hover={hover || isTablet}
        measure={'saturation'}
        ...
        title={'סטורציה'}
      />
      <Measure patient={patient} hover={hover || isTablet} measure={'pain'} icon={'pain'} title={'כאב'} />
    </div>
  );
};
