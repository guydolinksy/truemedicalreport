...
export const PatientMeasures = ({patient}) => {
    ...
    return <div style={{
        display: "flex", flexDirection: 'column', justifyContent: "space-between", padding: "8px 12px", maxHeight: 160,
        width: hover || isTablet ? '100%' : 40, flex: 0
    }} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        <Measure patient={patient} hover={hover || isTablet} measure={'temperature'} icon={'temperature'}
                 title={'חום'}/>
        <Measure patient={patient} hover={hover || isTablet} measure={'blood_pressure'} icon={'bloodPressure'}
                 title={'לחץ דם'}/>
        <Measure patient={patient} hover={hover || isTablet} measure={'pulse'} icon={'pulse'}
                 title={'דופק'}/>
        <Measure patient={patient} hover={hover || isTablet} measure={'saturation'} icon={'saturation'}
                 title={'סטורציה'}/>
        <Measure patient={patient} hover={hover || isTablet} measure={'pain'} icon={'pain'}
                 title={'כאב'}/>
    </div>
}