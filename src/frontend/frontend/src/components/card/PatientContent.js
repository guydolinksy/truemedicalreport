...
export const ProtocolStatus = () => {
  ...
  return value.protocol && value.protocol.items && value.protocol.items.length ? (
    value.protocol.items.map((item) => {
  ...
  return (
    ...
    >
        ...
        .map(([key, item]) => (
          ...
          >
            <Notification
              patient={patient}
              message={item}
              className={'patient-card-clickable-content'}
              style={{ whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflowX: 'hidden' }}
            />
          </div>
        ))}
      {Object.entries(value.notifications).slice(2, 3).length > 0 && (
        <div>
      ...
      {!Object.keys(value.notifications).length && (
        <Empty style={{ margin: -2 }} image={Empty.PRESENTED_IMAGE_SIMPLE} description={'אין עדכונים זמינים'} />
      )}
    </div>
  );
};
