...
export const PatientArrival = () => {
  ...
  useEffect(() => {
    if (moment().subtract(10, 'hours').isAfter(value.admission.arrival)) setArrivalClass('error-text');
    else if (moment().subtract(4, 'hours').isAfter(value.admission.arrival)) setArrivalClass('warn-text');
    ...
    >
      <div style={{ whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', overflowX: 'hidden' }}>
        ...
        <Tooltip
          overlay={
            <div>
              {value.intake.complaint}:<br />
              {value.intake.nurse_description}
            </div>
          }
        >
