...
export const MIN_WIDTH = 350, MAX_WIDTH = 500, MAX_HEIGHT = 250;
    ...
    return <div style={{display: 'flex', flexDirection: 'column', flex: 1, padding: '8px 12px'}}>
        ...
        <div style={{
            ...
            <Button key={destination.key} onClick={e => {
                update([MCI_DEPARTMENT, 'next_destination'], {
        ...
        <div style={{
            ...
            <Select style={{minWidth: 100, flex: 1}} value={next_destination} placeholder={'יעד הבא'}
                    ...
                    onSelect={value => update([MCI_DEPARTMENT, 'next_destination'], {
                        value: value,
                        at: moment().toISOString().replace('Z', '+00:00')
                    }, 'MCIStringValue')}/>
...
const PatientBody = ({mci, patient}) => {
    ...
    return mci ? <MCIPatientBody/> : <div style={{
        display: 'flex',
        ...
        <Carousel autoplay swipeToSlide draggable dotPosition={"botto"}>
            ...
            {Object.entries(value.warnings).filter(
                ...
                <PatientWarning key={i} patient={patient} warning={warning} index={i} contentStyle={{
                    direction: "rtl",
                    userSelect: "none",
                    cursor: "pointer",
                    padding: "8px 12px",
...
const PatientInner = ({patient, avatar, style, showAttention}) => {
    ...
    return <div className={`status-bar status-${value.status || 'unassigned'}`} style={{
        maxWidth: MAX_WIDTH, minWidth: MIN_WIDTH,
        maxHeight: MAX_HEIGHT, display: 'flex', flexDirection: "column", ...style
    }}>
...
export const Patient = ({patient, avatar, style, onError, showAttention}) => {
    const placeholder = (content) => <Card size={"small"} bodyStyle={{padding: 0}} headStyle={{
        paddingRight: -4, paddingLeft: -4
    }} title={<Button shape={"circle"} type={"text"}>{avatar || <UserOutlined/>}</Button>} style={{
        margin: 0,
        height: '100%',
        maxWidth: MAX_WIDTH,
        minWidth: MIN_WIDTH,
