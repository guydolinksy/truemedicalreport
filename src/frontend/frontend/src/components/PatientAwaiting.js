...
export const PatientAwaiting = () => {
    ...
    if (loading)
        return <Spin/>
    return <Space style={{paddingLeft: 20}}>{AWAITING.filter(k => value.awaiting[k]).map((k, i) => {
...
const PatientAwaitingIcon = ({awaitings, type}) => {
    ...
    useEffect(() => {
        if (Object.values(awaitings).some(({since, limit, completed_at}) =>
            !completed_at && moment().subtract(limit, "seconds").isAfter(since)))
            setStatus('error')
        else if (!Object.values(awaitings).some(({completed_at}) => !completed_at))
            setStatus('success')
        else
