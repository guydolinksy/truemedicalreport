...
export const HashMatch = ({ children }) => {
  ...
  const [matched, matching] = useMemo(() => {
    ...
    return [
      ...
      (match) =>
        parts.length >= match.length && !match.some((m, i) => m !== null && parts[i] !== m)
          ? parts.slice(match.length)
          : [],
    ];
  }, [hash]);

  return (
    <hashMatchContext.Provider value={{ matched: matched, matching: matching }}>{children}</hashMatchContext.Provider>
  );
};
