...
export const DisplayCard = () => {
  ...
  useEffect(() => {
    ...
    Axios.get('/api/settings/display', { cancelToken: s.token })
      .then((response) => {
  ...
  const onFinish = useCallback((values) => {
    Axios.post('/api/settings/display', values)
      .then(() => {
        setSuccess(true);
  ...
  return (
    <Card title={'הגדרות תצוגה'}>
      {success ? (
        <Alert message={'הגדרות התצוגה נשמרו בהצלחה'} type={'success'} closable afterClose={() => setSuccess(false)} />
      ) : (
        <Form
          ref={form}
          name={'display'}
