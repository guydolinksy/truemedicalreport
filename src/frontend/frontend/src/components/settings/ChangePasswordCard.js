...
export const ChangePasswordCard = () => {
  ...
  const onFinish = useCallback((values) => {
    Axios.post('/api/auth/change-password', values)
  ...
  return (
    <Card title={'הגדרות משתמש'}>
      {success ? (
        <Alert message={'הסיסמה הוחלפה בהצלחה'} type={'success'} closable afterClose={() => setSuccess(false)} />
      ) : (
        <Form name={'change-password'} title={'החלפת סיסמה'} onFinish={onFinish} onValuesChange={onChange}>
