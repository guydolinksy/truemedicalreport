...
const formatError = (prefix, { status, data }) => {
  return toParagraphs([prefix, `סטטוס: ${status}`, data.detail || data]);
};
...
const LDAPAuthentication = () => {
  ...
  useEffect(() => {
    ...
    Axios.get('/api/auth/ldap', { cancelToken: s.token })
      .then((response) => {
  ...
  const testUserErrorProps = missingTestUser
    ? { hasFeedback: true, validateStatus: 'error', help: 'נדרש שם משתמש לבדיקה' }
    : {};
  const testPasswordErrorProps = missingTestPassword
    ? { hasFeedback: true, validateStatus: 'error', help: 'נדרשת סיסמה למשתמש הבדיקה' }
    : {};
  ...
  return (
    ...
    >
      ...
      <Form.Item
        name={'uri'}
        label={'ניתוב שרת LDAP'}
        hasFeedback
        rules={[{ required: true, message: 'יש להזין את הניתוב לשרת LDAP' }]}
      >
        <Input placeholder={'ldap://...'} autoComplete={'off'} />
      </Form.Item>
      <Form.Item
        name={'users_base'}
        label={'מזהה בסיסי למשתמשים'}
        rules={[{ required: true, message: 'יש להזין מזהה בסיסי למשתמשים' }]}
      >
        <Input placeholder={'ex. ou=users,dc=example,dc=com'} autoComplete={'off'} />
      </Form.Item>
      <Form.Item
        name={'groups_base'}
        label={'מזהה בסיסי לקבוצות'}
        rules={[{ required: true, message: 'יש להזין מזהה בסיסי לקבוצות' }]}
      >
        <Input placeholder={'ex. ou=groups,dc=example,dc=com'} autoComplete={'off'} />
      </Form.Item>
      <Form.Item
        name={'use_ad_style_group_membership'}
        label={'מצב תאימות ל-Active Directory'}
      ...
      <Form.Item
        name={'filter'}
        label={'פילטר משתמשים'}
        rules={[{ required: true, message: 'יש להזין פילטר משתמשים' }]}
      >
        <Input placeholder={'sAMAccountName={username}'} autoComplete={'off'} />
      </Form.Item>
      <Form.Item
        name={'bind_dn'}
        label={'מזהה DN לחיבור'}
        rules={[{ required: true, message: 'יש להזין DN משתמש לחיבור' }]}
      >
      ...
      <Form.Item
        name={'bind_password'}
        label={'סיסמה לחיבור'}
        rules={[{ required: true, message: 'יש להזין סיסמה לחיבור' }]}
      >
      ...
      <Form.Item
        name={'admin_group_dn'}
        label={'קבוצת מנהלים'}
        rules={[{ required: true, message: 'יש להזין DN לקבוצת מנהלים' }]}
      >
      ...
      <Form.Item
        name={'user_group_dn'}
        label={'קבוצת משתמשים'}
        rules={[{ required: true, message: 'יש להזין DN לקבוצת משתמשים' }]}
      >
      ...
      <Form.Item>
        <Space>
          <Form.Item>
            <Button onClick={() => testLdapConfigGroupsOnly(form.current.getFieldValue())}>
              בדיקת קבוצות (ללא אימות)
            </Button>
          </Form.Item>
          <Form.Item>
