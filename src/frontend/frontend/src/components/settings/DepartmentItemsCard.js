...
export const DepartmentItemsCard = () => {
    ...
    const fetchFormattedData = (viewType, view, mode) => {
        Axios.get(`/api/settings/views/${viewType}/${view}/modes/${mode}/info/format`).then(response => {
    ...
    const handleSetValue = useCallback((path, newValue) => {
        const modifiedObject = modifyValue({...values}, newValue, path)
        setValues((prevValues) => ({...prevValues, ...modifiedObject}))
    }, [values])

    const handleDuplicateValue = useCallback((path, valueToDuplicate) => {
            ...
            {success ? <Alert message={'הגדרות התצוגה נשמרו בהצלחה'} type={"success"} closable
                ...
                <Form ref={form} name={"display"} title={'הגדרות הצגת פרטי מחלקה'} onFinish={onFinish}
                    ...
                    <Form.Item name={"config"} label={"תצורה:"}>
                        <RenderObject name={'components'} content={values.components} path={["components"]}
                                      setValue={handleSetValue} duplicateValue={handleDuplicateValue}
...
const RenderObject = ({name, content, setValue, duplicateValue, path, customRenderers = {}}) => {
    ...
    if (content.length)
        return <Card style={{width: 'fit-content'}} title={<span onClick={e => {
            setOpen(!open);
            e.stopPropagation()
        }}>
            {open ? <DownOutlined/> : <LeftOutlined/>} {convertToHebrew(name)}
        </span>} size={'small'} extra={<CopyOutlined onClick={e => {
            if (open) {
            ...
            {open && content.map((item, i) => <RenderObject key={i} content={item}
                                                            setValue={setValue}
                                                            path={path.concat(i)}
            ...
            {open && Object.entries(content).map(([k, v]) => {
                ...
                return <div key={k} style={{whiteSpace: 'nowrap'}}>
                    <Renderer name={k} content={v} setValue={setValue} path={path.concat(k)}
                              customRenderers={customRenderers} duplicateValue={duplicateValue}/>
                </div>
            })}
        </Card>
