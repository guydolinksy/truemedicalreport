...
const PatientNotification = ({ patient, markRead, read }) => {
  ...
  const last = patient.notifications
    .sort((k1, k2) => (moment(k1.at).isAfter(k2.at) ? 1 : -1))
    .slice()
    .pop();
  const worst = patient.notifications
    .sort((k1, k2) => (moment(k1.level).isAfter(k2.level) ? 1 : -1))
    .slice()
    .pop();
      ...
      header={
        <>
          <span className={`gender-${patient.info.gender}`}>
            <UserOutlined />
            {!user.anonymous && <span>&nbsp;{patient.info.name}</span>}
          </span>
          <br />
      ...
      extra={
        ...
        >
          ...
          <Space>
            ...
            {unread.length > 0 && <Badge className={badgeClass[worst]} count={unread.length} size={'small'} />}
          </Space>
        </div>
      }
    >
      ...
      {value.getPatients.patients.length ? (
        <div style={{ display: 'flex !important', flexDirection: 'column', flex: '1' }}>
          <Collapse onChange={openChange} style={{ flex: '1 0 10vh', minHeight: '10vh', overflowY: 'auto' }}>
            {value.getPatients.patients
              .filter((patient) => patient.notifications.length)
              .map((patient) => (
