            ...
            with self.session() as session:
                ...
                for row in result:
                    patients.append(ExternalPatient(
                        ...
                        esi=ESIScore(value=row["esi"], at=row["DepartmentAdmission"].astimezone().isoformat()),
                        admission=Admission(
                            department=department.name,
                            wing=row["DepartmentWing"],
                            bed=row["Bed_Name"],
                            arrival=row["DepartmentAdmission"].astimezone().isoformat(),
                        ),
                        intake=Intake(
    ...
    def update_treatment(self, department: Departments):
        ...
        try:
            ...
            with self.session() as session:
                ...
                for row in result:
                    treatments[row["id"]] = Treatment(destination=row["Name"] or row["Answer_Text"]). \
                        dict(exclude_unset=True)
