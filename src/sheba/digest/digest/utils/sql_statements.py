...
SELECT
    ...
    atd.Result,
    atd.Panic
FROM [sbwnd81c].[Chameleon].[dbo].[AuxiliaryTestOrders] AS ato
JOIN [sbwnd81c].[Chameleon].[dbo].[AuxTests] AS [at] ON ato.Test = [at].Code
JOIN [sbwnd81c].[Chameleon].[dbo].[MedicalRecords] AS mr ON ato.Patient = mr.Patient
JOIN [sbwnd81c].[Chameleon].[dbo].[RoomPlacmentPatient] AS rpp ON mr.medical_record = rpp.Medical_record
JOIN (
    SELECT MIN(o.Test_date) AS Order_Date, o.Order_Number
    from [sbwnd81c].[Chameleon].[dbo].[AuxiliaryTestOrders] AS o GROUP BY o.Order_Number
) AS d ON d.Order_Number = ato.Order_Number
LEFT OUTER JOIN  [sbwnd81c].[Chameleon].[dbo].[AuxiliaryTestDates] atd 
...
FROM [sbwnd81c].[Chameleon].[dbo].[ResponsibleDoctor] AS rd
JOIN [sbwnd81c].[Chameleon].[dbo].[Users] AS u ON rd.Doctor = u.Code
JOIN [sbwnd81c].[Chameleon].[dbo].[MedicalRecords] AS mr ON rd.Medical_Record = mr.Medical_Record
JOIN [sbwnd81c].[Chameleon].[dbo].[patients] AS pat ON pat.Patient = mr.Patient
JOIN [sbwnd81c].[Chameleon].[dbo].[RoomPlacmentPatient] AS rpp ON mr.Medical_Record = rpp.Medical_Record
WHERE
...
FROM [sbwnd81c].[Chameleon].[dbo].[Descriptions] AS d
JOIN [sbwnd81c].[Chameleon].[dbo].[MedicalRecords] AS mr ON d.Medical_Record = mr.Medical_Record
JOIN [sbwnd81c].[Chameleon].[dbo].[RoomPlacmentPatient] AS rpp ON mr.Medical_Record = rpp.Medical_Record
WHERE
